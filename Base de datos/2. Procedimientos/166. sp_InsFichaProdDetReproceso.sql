...
if exists (select name from sys.sysobjects WHERE name = 'sp_InsFichaProdDetReproceso')
begin 
  drop
    procedure sp_InsFichaProdDetReproceso
end
go

create procedure sp_InsFichaProdDetReproceso
  (
    @idFichaProd          int 
    , @idPartidaDet       int
    , @noPiezasPartida    int
